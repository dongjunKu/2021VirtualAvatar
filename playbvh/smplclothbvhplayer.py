...
def openBVH(motion_file_path):

    global mocap, joints, rotchannels, poschannels, total_frame
...
def getBVHFrame(frame):
    pose = np.zeros([24, 3])
    for bvhjoint in joints:
        try:
            if not (bvhjoint in bvh2smpl_jtmap):
                print(f"Unknown joint: {bvhjoint}")
                continue

            # 1.read angles and convert to radian
            angs = mocap.frame_joint_channels(frame, bvhjoint, rotchannels)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 5:
        print(
            'usage: %s base_path dataset idx bvhfile [videofile]' % sys.argv[0]), exit()
